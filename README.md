...
- Rediger tittel på TODO-listene
- Opprett og slett TODO-lister
...
- Hvis serveren krasjer, slutter hele applikasjonen å fungere. En peer-to-peer løsning ville ikke hatt dette problemet siden klientene kommuniserer direkte uten å være avhengig av én sentral server.
...
- Det er ingen låsing av oppgaver under redigering. Flere klienter kan redigere samme oppgave samtidig uten å se hverandres endringer, noe som kan føre til at én redigering overskriver en annen. En klient kan også slette en oppgave mens en annen klient er i ferd med å redigere den.
...
- Når to klienter redigerer samme oppgave samtidig, vinner én stille uten at brukeren varsles. Det er ingen synliggjøring av konflikter.
...
Åpne `http://localhost:3001` i nettleseren. Åpne samme adresse i flere faner for å se sanntidssynkroniseringen.
...
Testfiler: `tests/LWWRegister.test.ts`, `tests/ORSet.test.ts`, `tests/ListState.test.ts`.

## API-dokumentasjon

Ingen offentlig REST API. Kommunikasjon skjer over WebSocket med meldingsformatet definert i `src/protocol.ts`.

## Bruk av ekstern informasjon

- **Martin Kleppmann — "CRDTs: The Hard Parts"** (Strange Loop 2020) — konseptuelt grunnlag for add-wins-semantikk i ORSet og LWW-Register. <https://www.youtube.com/watch?v=x7drE24geUw>
- **Wikipedia — Conflict-free replicated data type** — oversikt over CRDT-typer og egenskaper. <https://en.wikipedia.org/wiki/Conflict-free_replicated_data_type>
- **Shapiro et al. — "A Comprehensive Study of Convergent and Commutative Replicated Data Types"** (INRIA 2011) — teknisk referanse for formelle definisjoner. <https://inria.hal.science/inria-00555588>

Ingen ekstern kode er kopiert. All implementasjon er egenutviklet.
